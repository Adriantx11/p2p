...
/**
 * Excepción personalizada para CurlX.
 */
class CurlXException extends Exception {}
...
class CurlX
{
     ...
     * @return object
     */
    public static function Get(string $url, array $headers=NULL, string $cookie=NULL, array $server=NULL ,string $userAgent='') : object
    {
        // Validar que la URL no sea null o vacía
        if (empty($url)) {
            throw new CurlXException("URL cannot be null or empty");
        }
        
        self::Prepare($url);
        self::SetOpt([CURLOPT_USERAGENT => $userAgent ?: self::userAgent()]);
     ...
     * @return object
     */
    public static function Post(string $url, $data=NULL, array $headers=NULL, string $cookie=NULL, array $server=NULL, string $userAgent='') : object
    {
        // Validar que la URL no sea null o vacía
        if (empty($url)) {
            throw new CurlXException("URL cannot be null or empty");
        ...
        self::Prepare($url);
        self::SetOpt([
            CURLOPT_USERAGENT      => $userAgent ?: self::userAgent(),
            CURLOPT_POST           => true,
     ...
     * @return void
     */
    public static function Custom(string $url, string $method='GET', $data=NULL, array $headers=NULL,string $cookie=NULL, array $server=NULL) : object 
    {
        // Validar que la URL no sea null o vacía
        if (empty($url)) {
            throw new CurlXException("URL cannot be null or empty");
...
/**
 * Send the request and handle the response with exceptions for specific cURL errors.
 *
 * @access public
 * @return object
 * @throws CurlXException
 */
public static function Run() : object
{
    try {
        self::MakeStdClass();
        self::SetOpt([
            CURLOPT_HEADERFUNCTION => createHeaderCallback(self::$headersCallBack),
        ]);
        self::$response = curl_exec(self::$ch);
        self::$info = curl_getinfo(self::$ch);

        // Obtiene el código HTTP final y la URL después de redirecciones
        $http_code = curl_getinfo(self::$ch, CURLINFO_HTTP_CODE);
        $final_url = curl_getinfo(self::$ch, CURLINFO_EFFECTIVE_URL);

        // Manejo de errores específicos de CURL
        if (self::$response === false) {
            $errorCode = curl_errno(self::$ch);
            $errorString = curl_error(self::$ch);
            curl_close(self::$ch);

            switch ($errorCode) {
                case CURLE_COULDNT_CONNECT:
                    throw new CurlXException("Could not connect to the server. Error code: {$errorCode}");
                case CURLE_OPERATION_TIMEDOUT:
                    throw new CurlXException("The request timed out. Error code: {$errorCode}");
                case CURLE_UNSUPPORTED_PROTOCOL:
                    throw new CurlXException("The protocol is not supported. Error code: {$errorCode}");
                default:
                    throw new CurlXException("Curl error occurred. Error code: {$errorCode} - {$errorString}");
            }
        }
        ...
        return (object) [
            'success' => true,
            'code'    => $http_code,
            'final_url' => $final_url, // URL final después de redirecciones
            'headers' => (object) [
        ...
        // Asegurarse de que el recurso CURL se cierre siempre
    ...
    {
        // Validar que $str no sea null
        if ($str === null) {
            return null;
        }
        
        // Divide la cadena usando la cadena de inicio
        $parts = explode($start, $str);
        
        // Verifica si la cadena de inicio se encontró
        if (count($parts) < 2) {
            return null; // Retorna null si la cadena de inicio no se encuentra
        }
        
        // Divide la parte resultante usando la cadena de fin
        $parts = explode($end, $parts[1]);
        
        // Verifica si la cadena de fin se encontró
        if (count($parts) < 2) {
            return null; // Retorna null si la cadena de fin no se encuentra
        }
        ...
        if ($decode) {
            ...
            if ($decoded === false) {
                return null; // Retorna null si la decodificación falla
            }
            return $decoded;
        }
